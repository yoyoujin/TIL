...
> Content Delivery Network의 약자로, 콘텐츠 전달 네트워크를 의미한다.

- 웹사이트의 콘텐츠(이미지, 스타일시트, 자바스크립트 파일 등)를 전 세계 여러 지역에 위치한 서버들을 통해 더 빠르고 안정적으로 전달하는 기술이다.
- 이를 통해 사용자들은 더 빠른 로딩 속도를 경험하고, 트래픽의 분산으로 웹사이트의 안정성도 향상시킬 수 있다.
  <br>

1. CDN의 동작원리
   CDN은 서버로부터 콘텐츠를 가져와 여러 지역에 위치한 에지 서버에 캐싱한다.
   사용자가 웹사이트에 접속하면,
   CDN은 가장 가까운 에지 서버로부터 콘텐츠를 전달한다.
   이로 인해 콘텐츠 로딩 속도가 빨라지는 이점이 있다.

```
한국의 사용자가 서버가 미국에 있는 페이지에 방문했다고 하자.
매번 미국 서버에 요청해서 페이지를 가져온다면 로딩이 오래 걸릴 것이다.

그러나 근접한 국가에 네트워크가 있다면
근접한 국가의 네트워크에서 먼저 데이터를 확인하고,
없다면 미국 서버에서 데이터를 받아와서 근접 네트워크에 캐시를 한다.

다음 요청시에는 미국 서버에서 요청하지 않고
해당 근접 네트워크에서 바로 데이터를 받아온다.
```

2. CDN의 장점

- 웹사이트의 성능을 향상시킬 수 있다.
- 빠른 로딩 속도는 사용자 경험을 향상시키며
- 더 많은 트래픽을 감당할 수 있게 도와준다.
- 특정 지역에 장애가 발생하더라도 다른 지역의 CDN서버를 통해 웹사이트의 콘텐츠에 접근할 수 있어 높은 가용성을 제공한다.

<br>

3. CDN의 캐싱과 업데이트

- CDN은 콘텐츠를 캐싱하여 제공하기 때문에 콘텐츠를 업데이트할 때 주의하여야하며
  캐싱 기간을 설정하거나, 버전관리를 통해 콘텐츠 업데이트를 원활하게 할 수 있다.
  <br>

4. 보안

- DDoS 공격 등 대규모 트래픽으로부터 보호될 수 있다.
- 일부 CDN 서비스는 HTTPS를 기본으로 지원하여 보안 통신을 강화할 수 있다.
  <br>

<hr>

이렇게 렌더링 방식들과 하이드레이션에 대해 정리해보았다.
우리의 앱은(momodo) 사용자 로그인이 있고,
데이터가 실시간으로 변경되는 서비스이기 때문에

SSR + CSR Hybrid 형태로 가져가게될 것 같다.
Next 13 부터는 서버 컴포넌트도 제공되기 때문에
어떤 컴포넌트에서 SSR 방식을 선택하고, CSR 방식을 선택할지
고민하여 결정해야겠다.

## TTV & TTI

앞서 렌더링 방식을 공부하면서 알게된 개념인
TTV와 TTI. 개발자에게 있어서 정말 중요한 포인트인데
너무 늦게 알게 된 것은 아닌지.. 🤔

프론트 개발자로서 염두하고 신경써야하는 부분인
`TTV`와 `TTI`에 대하여 정리해보기로 한다.

## TTV : Time To View

- 웹사이트가 사용자의 브라우저에 첫 화면을 보여주기 까지 걸리는 시간
- 웹사이트의 서버 응답 속도를 측정하는 데 사용됨
- SEO 측면에서도 중요한 지표 중 하나

## TTI : Time To Interact

- 웹사이트가 사용자와 상호작용할 수 있는 상태가 되기까지 걸리는 시간
- 웹페이지가 로딩되고, 모든 스크립트가 실행되어 사용자가 버튼이나 링크를 클릭하는 등의 상호작용을 할 수 있을 때까지의 시간을 말함
- 사용자 경험의 핵심적인 측면 중 하나

<hr>
TTI는 빠를수록 사용자는 더 빨리 웹사이트와 상호작용을 할 수 있고
이로인해 더 긍정적인 사용자 경험을 얻을 수 있을 것이다.

프론트엔드 개발자로서, 웹사이트의 로딩 속도를 최적화하여 TTV를 줄이고
TTI를 개선하는데 주력하며 개발을 해야할 것이다.

이를 위해 페이지 및 리소스 최적화, 코드 압축 등 다양한 기술과 방법을 고민해봐야할 것 같다.
